...
export default function Signin() {
	...
	async function emailSigin(e) {
		...
		try {
			...
			setErrorTitle("Your credentials are incorrect!");
			nProgress.done();
		}
	}
	...
	return (
		<>
			...
			<div className="w-screen h-screen flex justify-center items-center py-28 px-5">
				<div className="w-full border max-w-sm p-6 m-auto bg-white rounded-lg shadow-xl py-10">
					<form className="mt-6" onSubmit={emailSigin}>
						<div>
							<label
								htmlFor="username"
							...
							<input
								...
								value={username || ""}
								type="text"
								className="block w-full px-4 py-2 mt-2  bg-white border rounded-md  text-gray-800 border-gray-600  focus:border-violet-300 focus:ring-pink-300 focus:outline-none focus:ring focus:ring-opacity-40"
							/>
						</div>
						...
						<div className="mt-6">
							<button className="w-full px-4 py-2 tracking-wide text-white transition-all transform gradient font-semibold rounded-md hover:opacity-80 duration-200">
								Login
							</button>
						</div>
					</form>
